...
class FXOS8700CQR1(object):
	...
	def __init__(self):
		...
		while ((self._device.readU8(CTRL_REG2) & RST_MASK) == True):
			pass
		self._device.write8(M_CTRL_REG1, (HYBRID_ACTIVE|M_OSR2_MASK|M_OSR1_MASK|M_OSR0_MASK))	#OSR=max, Hybrid Mode 
		self._device.write8(M_CTRL_REG2, M_HYB_AUTOINC_MASK)							#Enable Hyb Mode Auto Increments  in order to read all data
		self._device.write8(CTRL_REG4, INT_EN_DRDY_MASK )								#Enable interrupts for DRDY (TO, Aug 2012)
		self._device.write8(XYZ_DATA_CFG, FULL_SCALE_2G)								#Full Scale of +/-2g
		self._device.write8(CTRL_REG1,(HYB_ASLP_RATE_25HZ|HYB_DATA_RATE_50HZ))			#System Output Data Rate of 200Hz (5ms), Sleep Mode Poll Rate of 50Hz (20ms)
	...
	def	getID (self):
		...
		:returns: none
		"""
		id = self._device.readU8(WHO_AM_I)
		return id
	...
	def	readStatusReg(self):
		...
		:returns: none
		"""
		return self._device.readU8(STATUS)
	...
	def activeMode(self):
		...
		:returns: none
		"""
		reg1 = self._device.readU8(CTRL_REG1)
		self._device.write8(CTRL_REG1, (reg1 | ACTIVE_MASK))			#Set the Active bit in System Control 1 Register.
	...
	def	standbyMode(self):
		...
		:returns: none
		"""
		n = self._device.readU8(CTRL_REG1)
		self._device.write8(CTRL_REG1, n & (~ACTIVE_MASK))
	...
	def	configureAccelerometer(self):
		...
		self.standbyMode()
		self._device.write8(CTRL_REG4, INT_EN_DRDY_MASK )          		# Enable interrupts for DRDY (TO, Aug 2012)
		self._device.write8(XYZ_DATA_CFG, FULL_SCALE_2G)           		# Set FSR of accel to +/-2g
		self._device.write8(CTRL_REG1, (HYB_ASLP_RATE_25HZ|HYB_DATA_RATE_50HZ))     # Set ODRs
		self.activeMode()
	...
	def configureMagnetometer(self):
		...
		"""
		self.standbyMode()
		self._device.write8(M_CTRL_REG1, (HYBRID_ACTIVE|M_OSR2_MASK|M_OSR1_MASK|M_OSR0_MASK))    # OSR=max, hybrid mode (TO, Aug 2012)
		self._device.write8(M_CTRL_REG2, M_HYB_AUTOINC_MASK)       #enable hybrid autoinc
		self.activeMode()
	...
	def pollAccelerometer (self):
		...
		:returns: none
		"""
		raw0 = self._device.readU8(OUT_X_MSB)   
		raw1 = self._device.readU8(OUT_X_LSB)
		raw2 = self._device.readU8(OUT_Y_MSB)
		raw3 = self._device.readU8(OUT_Y_LSB)
		raw4 = self._device.readU8(OUT_Z_MSB)
		raw5 = self._device.readU8(OUT_Z_LSB)
	...
	def	FIFOMode(self,mode):
		...
		:returns: none
		"""
		self._device.write8(F_SETUP,6<<mode)
	...
	def	highPassFilter(self,status):
		...
		:returns: none
		"""
		self._device.write8(XYZ_DATA_CFG,status)
	...
	def	fullScaleRange(self,range):
		...
		:returns: none
		"""
		self._device.write8(XYZ_DATA_CFG,range)
	...
	def	setAccelerometerDynamicRange(self,range):
		...
		self.standbyMode() 	
		if range == SCALE2G:
				self._device.write8(XYZ_DATA_CFG, (self._device.readU8(XYZ_DATA_CFG) & ~FS_MASK))		#Write the 2g dynamic range value into register = 0x0E		 
		elif range == SCALE4G:
				self._device.write8(XYZ_DATA_CFG, (self._device.readU8(XYZ_DATA_CFG) & ~FS_MASK)) 		#Write the 4g dynamic range value into register = 0x0E
				self._device.write8(XYZ_DATA_CFG, (self._device.readU8(XYZ_DATA_CFG) | FULL_SCALE_4G)) 		 
		elif range ==SCALE8G:
				self._device.write8(XYZ_DATA_CFG, (self._device.readU8(XYZ_DATA_CFG) & ~FS_MASK)) 		#Write the 8g dynamic range value into register = 0x0E
				self._device.write8(XYZ_DATA_CFG, (self._device.readU8(XYZ_DATA_CFG) | FULL_SCALE_8G)) 
		self.activeMode() 
	...
	def	magnetometerStatus(self):
		...
		:returns: none
		"""
		stat = self._device.readU8(M_DR_STATUS)
		return stat
	...
	def	pollMagnetometer (self):
		...
		:returns: none
		"""
		raw0 = self._device.readU8(M_OUT_X_MSB)
		raw1 = self._device.readU8(M_OUT_X_LSB)
		raw2 = self._device.readU8(M_OUT_Y_MSB)
		raw3 = self._device.readU8(M_OUT_Y_MSB)
		raw4 = self._device.readU8(M_OUT_Z_MSB)
		raw5 = self._device.readU8(M_OUT_Z_MSB)	
	...
	def	setODR (self,DataRateValue):
		...
		self.standbyMode() 
		self._device.write8(CTRL_REG1,self._device.readU8(CTRL_REG1) & ~DR_MASK)		#Write in the Data Rate value into Ctrl Reg 1 
		self._device.write8(CTRL_REG1, self._device.readU8(CTRL_REG1)| DataRateValue)
		self.activeMode() 
	...
	def getOrientation(self):
		...
		:returns: none
		"""
		orientation = self._device.readU8(PL_STATUS)
		return orientation
	...
	def configureOrientation(self):
		...
		:returns: none
		"""
		CTRL_REG1_Data = self._device.readU8(0x2A)			#read contents of register 
		CTRL_REG1_Data &= 0xFE									#Set last bit to 0. 
		self._device.write8(0x2A, CTRL_REG1_Data)			 

		CTRL_REG1_Data = self._device.readU8(0x2A) 			#Note: Can combine this step with above 
		CTRL_REG1_Data &= 0xC7								#Clear the sample rate bits 
		CTRL_REG1_Data |= 0x20								#Set the sample rate bits to 50 Hz 
		self._device.write8(0x2A, CTRL_REG1_Data) 		#Write updated value into the register. 

		#Set the PL_EN bit in Register 0x11 PL_CFG. This will enable the orientation detection. 
		PLCFG_Data = self._device.readU8(0x11)
		PLCFG_Data |= 0x40 
		self._device.write8(0x11, PLCFG_Data)

		#Set the Back/Front Angle trip points in register 0x13 following the table in the data sheet.

		PL_BF_ZCOMP_Data = self._device.readU8(0x13)
		...
		self._device.write8(0x13, PL_BF_ZCOMP_Data)	#Write in the updated Back/Front Angle
		PL_BF_ZCOMP_Data = self._device.readU8(0x1C) 	#Read out contents of the register (can be read by all 
		PL_BF_ZCOMP_Data &= 0xF8 						#Clear the last three bits of the register 
		PL_BF_ZCOMP_Data |= 0x00 
		
		self._device.write8(0x13, PL_BF_ZCOMP_Data)		#Write in the updated Z-lockout angle
		P_L_THS_Data = self._device.readU8(0x14)					
		P_L_THS_Data &= 0x07		#Clear the Threshold values 
		P_L_THS_Data |= (0x07)<<3
		self._device.write8(0x14,P_L_THS_Data)
		...
		CTRL_REG4_Data = self._device.readU8(0x2D)	#Read out the contents of the register
		CTRL_REG4_Data |= 0x10							#Set bit 4 
		self._device.write8(0x2D, CTRL_REG4_Data)		#Set the bit and write into CTRL_REG4
		
		CTRL_REG5_Data = self._device.readU8(0x2E)	 	
		...
		self._device.write8(0x2E, CTRL_REG5_Data)		#Write in the interrupt routing selection 
		
		self._device.write8(0x12, 0x05)				#This sets the debounce counter to 100 ms at 50 Hz 
		CTRL_REG1_Data = self._device.readU8(0x2A)		#Read out the contents of the register 
		CTRL_REG1_Data |= 0x01							#Change the value in the register to Active Mode. 
		self._device.write8(0x2A, CTRL_REG1_Data)		#Write in the updated value to put the device in Active Mode 
	...
	def getChipMode(self):
		...
		:returns: none
		"""
		mode = self._device.readU8(SYSMOD)
		return mode
	...
	def ConfigureGenericTapMode(self):
		"""
		...
		:param 
		"""
		self._device.write8(PULSE_CFG, 0x55)	#Example X, Y and Z configured for Single Tap with Latch enabled
		#self._device.write8(PULSE_CFG, 0x6A)	#Example X, Y and Z configured for Double Tap with Latch enabled
		#self._device.write8(PULSE_CFG, 0x7F)	#Example X, Y and Z configured for Single Tap and Double Tap with Latch enabled
	
		#**************Set Threesholds*************************/
		self._device.write8(PULSE_THSX, 0x20) 	#Set X Threshold to 32 counts or 2g
		self._device.write8(PULSE_THSY, 0x20) 	#Set Y Threshold to 32 counts or 2g
		self._device.write8(PULSE_THSZ, 0x0C) 	#Set Z Threshold to 48 counts or 3g
	
		self._device.write8(PULSE_TMLT, 0x06)	#Set the Pulse Time Limit for 30 ms at 200 Hz ODR in Normal Mode with the LPF Enabled A. 30 ms/5 ms = 6 counts
		self._device.write8(PULSE_LTCY, 0x28)	#Set the Pulse Latency Timer to 200 ms, 200 Hz ODR Low Power Mode, LPF Not Enabled. 200 ms/5.0 ms = 40 counts
		self._device.write8(PULSE_WIND, 0x0F)	#Set the Pulse window to 300 ms, 100 Hz ODR Low Power Mode, LPF Enabled 300 ms/20 ms = 15 counts	
	
		self._device.write8(CTRL_REG4, 0x08)	#Enable Tap Interrupt in Register 0x2D.
		self._device.write8(CTRL_REG5,0x08) 	#Set Tap to INT1


	def ConfigureSingleTapMode(self):
		"""
		Configure Single Tap mode
		...
		self._device.write8(CTRL_REG1, 0x08) 					#400 Hz, Standby Mode
		self._device.write8(PULSE_CFG, 0x15)					#Enable X and Y and Z Single Pulse
		self._device.write8(PULSE_THSX, 0x19) 				#Set X Threshold to 1.575g
		self._device.write8(PULSE_THSY, 0x19) 				#Set Y Threshold to 1.575g 
		self._device.write8(PULSE_THSZ, 0x2A) 				#Set Z Threshold to 2.65g
		self._device.write8(PULSE_TMLT,0x50)  				#Set Time Limit for Tap Detection to 50 ms, Normal Mode, No LPF
		self._device.write8(PULSE_LTCY,0xF0) 					# Set Latency Time to 300 ms
		self._device.write8(CTRL_REG4, 0x08) 					#Route INT1 to System
		self._device.write8(CTRL_REG5, 0x08) 					#Route Pulse Interrupt Block to INT1 hardware Pin 
		CTRL_REG1_Data = self._device.readU8(CTRL_REG1)		#Read out the contents of the register
		CTRL_REG1_Data |= 0x01 								#Change the value in the register to Active Mode.
		self._device.write8(CTRL_REG1, CTRL_REG1_Data) 		#Write in the updated value to put the device in 



	def FXOS8700CQ_ConfigureDoubleTapMode(self):
		"""
		
		:param DoubleTap_LowPowerMode_NoLPF_400HzODR
		"""
		self._device.write8(CTRL_REG1, 0x08) 							#400 Hz, Standby Mode
		self._device.write8(PULSE_CFG, 0x2A)							#Enable X, Y and Z Double Pulse with DPA = 0 no double pulse abort
		self._device.write8(PULSE_THSX, 0x08) 						#Set X Threshold to 3g 
		self._device.write8(PULSE_THSY, 0x08) 						#Set Y Threshold to 3g
		self._device.write8(PULSE_THSZ, 0x03) 						#Set Z Threshold to 5g
		self._device.write8(PULSE_TMLT,0x30) 							#60 ms Note: 400 Hz ODR, Time step is 1.25 ms per step
		self._device.write8(PULSE_LTCY,0x50)							#200 ms Set Latency Time to 200 ms
		self._device.write8(PULSE_WIND,0x78) 							#300 ms Set Time Window for second tap to 300 ms
		self._device.write8(CTRL_REG4, 0x08) 							#Enable Pulse Interrupt in System CTRL_REG4
		self._device.write8(CTRL_REG5, 0x08) 							#Route Pulse Interrupt to INT1 hardware Pin CTRL_REG5

		CTRL_REG1_Data = self._device.readU8(CTRL_REG1) 	#Read out the contents of the register
		CTRL_REG1_Data |= 0x01 										#Change the value in the register to Active Mode.
		self._device.write8(CTRL_REG1, CTRL_REG1_Data) 				#Write in the updated value to put the device in Active Mode.
